...
contract Keepr is ReentrancyGuard, Pausable, Ownable {
    ...
    // State variables
    Counters.Counter private _keepIds;
    ...
    modifier onlyKeepCreator(uint256 keepId) {
        require(keeps[keepId].creator == msg.sender, "Only creator can perform this action");
        _;
    }
    ...
    modifier keepUnlocked(uint256 keepId) {
        require(block.timestamp >= keeps[keepId].unlockTime, "Keep is not yet unlocked");
        _;
    }

    // Constructor
    constructor() {
        _keepIds.increment(); // Start from 1
    }
    ...
    function createKeep(
        ...
        require(unlockTime > block.timestamp + minUnlockDelay, "Unlock time too soon");
        require(unlockTime <= block.timestamp + maxUnlockDelay, "Unlock time too far");
        require(msg.value >= platformFee, "Insufficient platform fee");

        uint256 keepId = _keepIds.current();
        _keepIds.increment();
     ...
     */
    function claimKeep(uint256 keepId) 
        external 
        nonReentrant 
        keepExists(keepId) 
        keepActive(keepId) 
     ...
     */
    function activateFallback(uint256 keepId) 
        external 
        nonReentrant 
        keepExists(keepId) 
        keepActive(keepId) 
    ...
    {
        Keep storage keep = keeps[keepId];
        require(keep.fallbackRecipient == msg.sender, "Only fallback recipient can activate");
        require(keep.recipient != address(0), "No primary recipient to fallback from");
     ...
     */
    function cancelKeep(uint256 keepId) 
        external 
        nonReentrant 
        onlyKeepCreator(keepId) 
        keepActive(keepId) 
    {
        Keep storage keep = keeps[keepId];
        require(block.timestamp < keep.unlockTime, "Cannot cancel after unlock time");
     ...
     */
    function changeRecipient(uint256 keepId, address newRecipient) 
        external 
        nonReentrant 
        onlyKeepCreator(keepId) 
        keepActive(keepId) 
    {
        Keep storage keep = keeps[keepId];
        require(block.timestamp < keep.unlockTime, "Cannot change after unlock time");
